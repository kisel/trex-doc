...
- {"vtype": "BYTES", "generate": ... } - generate bytes payload. see below
...
==== Generating binary payload

[source,python]
----
# generate 64 bytes of printable ascii payload
# seed is optional number to define a pseudo-random number sequence
- {"vtype": "BYTES", "generate": "random_ascii", "size": 64, "seed": 12345}

# generate 32 bytes(from 0 to 0xFF bytes), using default seed value
- {"vtype": "BYTES", "generate": "random_bytes", "size": 32}

# generate 32 bytes using repeating/cut specified template
- {"vtype": "BYTES", "generate": "template", "size": 32, template_base64: "<base64 string>"}

# same as generate from template, but template is specified with a hex string
# spaces and 0x are ignored. only 2 character hex values currently allowed
- {"vtype": "BYTES", "generate": "template_code", "size": 8, template_code: "DE AD BE EF 13 10"}

# generates payload from hex values. for this case size can be omitted(it will be calculated automatically)
- {"vtype": "BYTES", "generate": "template_code", template_code: "DE AD BE EF 13 10"}

# generate random bytes to fill the packet size to 128 bytes
# you can also replace 'size' parameter with a 'total_size' in any examples above.
# generator size or total_size parameter to be specified.
#There should be no multiple payloads with total_size
- {"vtype": "BYTES", "generate": "random_bytes", "total_size": 128}
----
...
* You should NOT run scapy_server as root - use a user with a limited access
...
You may need to support custom structures
* The server can be shut down using the keyboard interrupt Ctrl+C
* The server can be shut down remotely with the method "shut_down" with no arguments
...
=== Extending Scapy Server
Scapy Server can automatically generate definition from Scapy classes.

==== Extending UI definitions
protocols.json file in a scapy_server directory defines look&feel for UI for particular protocols and fields.
By modifying it you can
- alter field/protocol labels and enums
- add field value restriction(min/max for numbers, regular expressions)
- change how the filed will be rendered( type: NUMBER/BYTES/STRING/ENUM/EXPRESSION etc...)
- add enums to display possible choices in a UI

Examples:
[source,python]
----
# add a restriction for VXLAN.vni to accept only '1' or '7' strings using regular expression
{
    "id": "VXLAN",
    "fields": [
        { "id": "vni", "regex": "^[17]$" }
...
# as an alternative, we can use an enum, which suggests possible values, but does not limit them(so any value can be entered)
{
    "id": "VXLAN",
        "fields": [
        {
...
# limit TCP.sport values to a number range of 0-65535
{
    "id": "TCP",
        "fields": [
        {
...
==== Adding new procotols support
You can also extend the Scapy server by adding new Scapy classes.
All loaded Scapy classes derived from scapy.all.Packet will appear in the Scapy Server, so you can create and load your extension modules. New classes can also be tweaked using 'protocols.json. file

Example:
[source,python]
----
# scapy_extensions.py
# this file needs to be included into a scapy_service.py
